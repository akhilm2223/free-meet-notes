...
// Compatibility commands retained for inherited frontend call sites. Free Meet
// Notes deliberately has no analytics client, project key, or network transport.

#[command]
pub async fn init_analytics() -> Result<(), String> {
    Err("Usage analytics is disabled in Free Meet Notes".to_string())
}
