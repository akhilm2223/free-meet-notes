...
fn download_and_extract_ffmpeg(
    ...
    // Get platform-specific download URL and the digest published with release 0.0.1.
    let (url, expected_sha256) = get_ffmpeg_release_for_target(target)?;
    ...
    // Write the verified archive to a temp file.
...
/// Get FFmpeg download URL and its published SHA-256 digest for a target triple.
/// Digests come from GitHub's release metadata for Zackriya-Solutions/ffmpeg-binaries 0.0.1.
fn get_ffmpeg_release_for_target(target: &str) -> Result<(&'static str, &'static str), String> {
    let release = if target.contains("windows") {
        // Windows
        (
